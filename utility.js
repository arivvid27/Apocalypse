...
function displayText() {
    ...
    fill(255);
    t2 = "Wave : " + (waveNumber + 1);
    t3 = "Zombies Left : " + (enemies.length + currentWave.total);
    fill(35);
    noStroke();
    rect(width - 25 - textWidth(t3), 5, textWidth(t3) + 25, 80,5);
    fill(255);
    if (currentWave.total == Infinity) {
        t3 = "Zombies Left : " + currentWave.total;
    } else {
        t3 = "Zombies Left : " + (enemies.length + currentWave.total);
    }
    text(t, width - 20 - textWidth(t3), 20);
    text(t2, width - 20 - textWidth(t3), 45);
    text(t3, width - 20 - textWidth(t3), 70);
    pop();
}
